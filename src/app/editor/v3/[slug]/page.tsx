...
export default function EditorSlugPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        const res = await fetch(`/api/articles/v2/${encodeURIComponent(slug)}`);
  ...
  async function handlePublish() {
    ...
    setPublishing(true);
    try {
      const res = await fetch(`/api/articles/v2/${encodeURIComponent(slug)}/publish`, { method: "PATCH" });
      const data = await res.json();
  ...
  async function handleDraft() {
    ...
    try {
      const res = await fetch(`/api/articles/v2/${encodeURIComponent(slug)}/draft`, { method: "PATCH" });
      const data = await res.json();
  ...
  async function saveScratchPadNow() {
    if (!slug) return;
    try {
      await fetch(`/api/articles/v2/${encodeURIComponent(slug)}/scratchpad`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  async function saveNotesNow() {
    if (!slug) return;
    try {
      await fetch(`/api/articles/v2/${encodeURIComponent(slug)}/notes`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-zinc-950 via-zinc-950 to-black text-zinc-100">
      ...
      <div className="border-b border-zinc-900/80 bg-zinc-950/80 backdrop-blur supports-[backdrop-filter]:bg-zinc-950/60 sticky top-0 z-30">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                className="text-base md:text-lg font-semibold bg-transparent border border-zinc-800 focus:border-emerald-600 rounded-md px-2 py-1 text-white w-[42vw]"
              />
        ...
        {/* Main Tab Bar */}
        <div className="border-t border-zinc-900/70 bg-zinc-950/50">
          <div className="max-w-7xl mx-auto px-4">
            <div className="flex flex-wrap gap-1 py-2">
              {([
                ...
                { key: "scratchpad", label: "ScratchPad", icon: FileText },
                { key: "resources", label: "Resources", icon: LinkIcon },
                { key: "references", label: "References", icon: BookOpen },
                { key: "revisions", label: "Revisions", icon: ListTree },
              ] as const).map(({ key, label, icon: Icon }) => (
                <button
                  key={key}
                  onClick={() => setActiveTab(key)}
                  className={cx(
                    "inline-flex items-center gap-2 px-3 py-2 rounded-md text-sm transition border",
                    activeTab === key
                      ? "border-emerald-600/60 bg-emerald-600/10 text-emerald-300"
                ...
                <div className="md:col-span-2">
                  ...
                  {previewMode === "edit" ? (
                    <textarea
                      ...
                      className="w-full min-h-[110px] mt-2 bg-zinc-900/40 border border-zinc-800 rounded-md p-3 text-sm text-zinc-200 resize-y"
                    />
                  ) : (
                    <div className="mt-2 p-3 border border-zinc-800 rounded-md bg-zinc-900/40 text-sm text-zinc-200 leading-6">
                      {abstractText || <span className="text-zinc-500">No abstract yet.</span>}
                    </div>
                  )}
                </div>
                <div>
                  <label className="text-xs text-zinc-400 flex items-center gap-2">
                    <KeyRound className="w-3 h-3" /> Keywords
                  </label>
                  <input
                  ...
                  <div className="text-[11px] mt-2 text-zinc-500">
                    Tip: Use 5–10 specific keywords for discoverability (max 12 stored).
                  </div>
                </div>
              </section>
