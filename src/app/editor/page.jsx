...
export default function EditorPage() {
  ...
  async function handleSaveDraft() {
    ...
    const payload = {
      ...
      blocks,
      scratchpad, // Add scratchpad content
    ...
    try {
      const resp = await fetch("/api/articles/drafts", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      setId(json.article._id);
      setMessage("Draft saved successfully");
      router.push(`/editor/${json.article.slug}`);
    } catch (err) {
      console.error(err);
  ...
  async function handlePublish() {
    if (!id) {
      setMessage("Please save draft before publishing");
      return;
    }
    ...
    const resp = await fetch("/api/articles/publish", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id, publishedBy: "Your Name" }),
    });
    const json = await resp.json();
    if (!json.success) {
      setMessage("Publish failed: " + (json.error || "error"));
      return;
    }
  ...
  async function handleSaveScratchpad() {
    if (!id) {
      setMessage("Please save draft first to save scratchpad");
      return;
    }

    setSaving(true);
    setMessage("Saving scratchpad...");
    ...
    try {
      ...
      const json = await resp.json();
      if (!json.success)
        throw new Error(json.error || "Failed to save scratchpad");
      setMessage("Scratchpad saved successfully");
    } catch (err) {
      console.error(err);
      setMessage("Scratchpad save failed: " + err.message);
    } finally {
      setSaving(false);
    }
  }
      ...
      <div className="max-w-7xl mx-auto px-8 py-6">
        <div className="grid grid-cols-4 gap-8">
          ...
          <div className="col-span-3">
            ...
            {/* Rest of the tabs (citations, seo, resources) remain the same */}
            {activeTab === "citations" && (
              <div className="bg-[#1a1c23] border border-gray-800 rounded-xl p-6">
                <div className="flex justify-between items-center mb-6">
                  <h3 className="text-lg font-semibold text-white">
