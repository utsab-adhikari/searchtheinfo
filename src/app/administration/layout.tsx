...
const NAV_ITEMS = [
  ...
  { label: 'System Settings', icon: Settings, href: '/administration/settings' },
];
...
export default function AdministrationLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-300 flex relative overflow-hidden">
      ...
      <aside className={`fixed lg:relative z-50 h-screen bg-zinc-950/90 border-r border-zinc-800/30 backdrop-blur-xl transition-all duration-300 ease-in-out ${
        collapsed ? 'w-20' : 'w-64'
      } ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'} top-16 lg:top-0`}>
        <div className="flex flex-col h-full relative">
          ...
          <div className={`p-4 ${collapsed ? 'px-4' : ''}`}>
            <Link href="/" className={`flex items-center gap-2 text-lg font-bold text-white ${collapsed ? 'justify-center' : ''}`}>
              {collapsed ? (
          ...
          <nav className={`space-y-1 flex-1 overflow-y-auto px-2 py-4 ${collapsed ? 'items-center' : ''}`}>
            {NAV_ITEMS.map((item) => {
              ...
              return (
                <Link
                  ...
                  onClick={() => setSidebarOpen(false)}
                >
                  <item.icon className={`w-4 h-4 ${isActive ? 'text-emerald-400' : 'text-zinc-500 group-hover:text-emerald-400 transition-colors'}`} />
          ...
          <div className={`p-4 border-t border-zinc-800/30 ${collapsed ? 'px-3' : ''}`}>
             {!collapsed && (
             ...
             <button 
               ...
               className={`flex items-center gap-2 w-full px-3 py-2 rounded-lg text-[13px] font-medium group transition-colors ${collapsed ? 'justify-center' : ''}`}
             >
               <LogOut className={`w-4 h-4 ${collapsed ? 'mx-auto' : ''} text-zinc-400 group-hover:text-rose-400 transition-colors`} />
               {!collapsed && <span className="text-zinc-400 group-hover:text-rose-400">Sign Out</span>}
             </button>
          </div>
        </div>
      </aside>
